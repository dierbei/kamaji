...
create-tag: # Add a tag to the current commit and push it to the remote warehouse.
...
# - use the VERSION as arg of the bundle target (e.g make release VERSION=0.4.0)
# - use environment variables to overwrite this value (e.g export VERSION=0.4.0 & make release)
release: create-tag
	GIT_REPO=$(GIT_REPO) GIT_HEAD_COMMIT=$(GIT_HEAD_COMMIT) VERSION=$(VERSION) GIT_MODIFIED=$(GIT_MODIFIED) BUILD_DATE=$(BUILD_DATE) goreleaser release --clean || \
